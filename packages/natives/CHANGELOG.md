...
- Windows addon staging is disabled because the runtime only loads N-API addons by pathname and cannot bind a verified file descriptor to the loaded module. Installed-package updates can retain the platform's existing update-lock behavior while a process is running.
