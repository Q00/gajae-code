...
- Windows staged-addon refresh files are now content-addressed and pruned after a newer verified refresh is published, preventing repeated in-place package updates from accumulating unbounded cache artifacts while preserving active-process files when the platform keeps them open.
